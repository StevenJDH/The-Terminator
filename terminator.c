...
int argsCLI(int argc, char** argv) {
	...
	while((option = getopt(argc, argv, "-k:K:r:R:n:N:d:D:hH?")) != -1) { // Colons require value with option, dash requires options with value.
		switch (option)
		{
			case 'K':
			case 'k':
				if (processName != NULL) { return printUsage(argv[0]); } // Ensures option flag is only used once.
				processName = optarg;
				break;
			case 'R':
			case 'r':
				if (filename != NULL || optMode != OPT_KILL) { return printUsage(argv[0]); } // Ensures only one mode is set.
				filename = optarg;
				optMode = OPT_KILL_RENAME;
				break;
			case 'N':
			case 'n':
				if (newFilename != NULL) { return printUsage(argv[0]); }
				newFilename = optarg;
				break;
			case 'D':
			case 'd':
				if (filename != NULL || optMode != OPT_KILL) { return printUsage(argv[0]); }
				filename = optarg;
